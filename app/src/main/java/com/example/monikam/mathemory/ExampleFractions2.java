...
public class ExampleFractions2 extends AppCompatActivity implements View.OnTouchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_example_fractions2);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: // puszczenie pola
                ...
                if ((Math.abs(v.getY() - target1.getY()) < 50 && Math.abs(v.getX() - target1.getX()) < 50) && (v.getId() == R.id.number2)) {
                        v.animate().x(centreX1).y(centreY1).setDuration(0).start(); // umieszczenie w pierwszym kwadracie

                }
                else if ((Math.abs(v.getY() - target2.getY()) < 50 && Math.abs(v.getX() - target2.getX()) < 50) && (v.getId() == R.id.number1)) {
